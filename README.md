...
- **포즈 1 (1단계)** — 스툴에 앉아 정면, 한 다리는 바닥으로 뻗고 다른 무릎은 세움. 힌트 18/18, 사슬 완전, 교차 없음.
  → 프롬프트: *짧은 보헤미안 드레스를 입은 아프리카 여성, 맨다리와 가죽 샌들, 햇빛 드는 콘크리트 갤러리, 85mm f/2*
  → 결과 `output_01.png`, **관절 오차 0.022** (최악 r_ankle 0.042) → **잘 됨**
- **포즈 1, 프롬프트만 변경 (2단계-A)** → 프롬프트: *잉크와 수채로 그린 유랑 인형극사, 크림색 종이, 오커와 번트 엄버*
  → 결과 `output_01_alt_prompt.png`, **오차 0.046** (최악 r_ankle 0.160) → **잘 됨**, 다만 사실성에서 멀어지면 추종이 약해짐
- **포즈 2, 프롬프트 고정 (2단계-B)** — 서서 정면, 양 팔뚝을 올려 가슴 높이에서 주먹 쥠, 발은 넓게. 힌트 18/18, 사슬 완전, 교차 없음.
  → 프롬프트: 포즈 1과 바이트 단위로 동일 → 결과 `output_02.png`, **오차 0.045** (최악 r_knee 0.144) → **잘 됨**

**핵심 대조**: 프롬프트를 바꾸면 자세만 남고 나머지가 전부 바뀌었고, 포즈 사진을 바꾸면 자세만
바뀌었습니다. 이 비대칭이 결론입니다 — 스켈레톤이 신체 위치를, 프롬프트가 신체 정체성을 담당합니다.
`output_01.png`과 `output_02.png`는 프롬프트·시드가 바이트 단위로 같으므로, 두 이미지의 차이는
스켈레톤 출처 하나로만 설명됩니다.

**어긋난 부분 하나**: 같은 프롬프트가 짧은 드레스를 요구했는데 앉은 자세에는 짧게, 서 있는 자세에는
종아리를 덮는 길이로 그려졌습니다. `output_02.png`의 최악 관절이 무릎(0.144)인 것이 그 지점입니다.
옷 기장은 프롬프트로 안정적으로 통제되지 않고, 옷이 관절을 덮으면 재검출이 추정에 의존해 오차가 오릅니다.
...
- **프롬프트의 절들이 균등하게 반영되지 않습니다.** 구체적인 쪽이 이깁니다 — 재질과 광학 현상을
  함께 적은 절은 그려지고, 추상 명사 하나만 있는 절은 무시되는 경향이 있습니다. 또한 텍스트
  인코더가 77토큰에서 **뒤쪽부터** 조용히 잘라내므로, 프롬프트를 수정할 때마다 토크나이저로
  확인해야 합니다.
